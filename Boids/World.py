...
class World(object):
        ...
        if self.rangeClustering == 0:
            ...
            for i in range(0, len(self.boids)-1):
                for k in range(i+1, len(self.boids)):
                    difference = [self.boids[i].position[0] - self.boids[k].position[0], self.boids[i].position[1] - self.boids[k].position[1]]
                    if difference[0]**2 + difference[1]**2 < self.boidConfig[0]:
                        if self.boids[i].angleBetweenBoids(self.boids[i].velocity, difference) < self.boids[i]._viewAngle:
                            self.boids[i]._boidMask[k] = 1 
                        if self.boids[k].angleBetweenBoids(self.boids[k].velocity, difference) < self.boids[k]._viewAngle:
            ...
            for i in range(n_clusters_):            
                for k in range(0, len(groups[i])-1):
                    for j in range(k+1, len(groups[i])):
                        difference = [self.boids[groups[i][k]].position[0] - self.boids[groups[i][j]].position[0], self.boids[groups[i][k]].position[1] - self.boids[groups[i][j]].position[1]]
                        if difference[0]**2 + difference[1]**2 < self.boidConfig[0]:
                            if self.boids[groups[i][k]].angleBetweenBoids(self.boids[groups[i][k]].velocity, difference) < self.boids[groups[i][k]]._viewAngle:
                                self.boids[groups[i][k]]._boidMask[groups[i][j]] = 1 
                            if self.boids[groups[i][j]].angleBetweenBoids(self.boids[groups[i][j]].velocity, difference) < self.boids[groups[i][j]]._viewAngle:
            ...
            for i in range(0, len(self.boids)):
                x = int(self.boids[i].position[0] // (self.width / numTilesW))
                x = np.min([numTilesW-1,x])
                y = int(self.boids[i].position[1] // (self.height / numTilesH))
            ...
            x = int(self.boids[0].position[0] // (self.width / numTilesW)) * (self.width / numTilesW)
            y = int(self.boids[0].position[1] // (self.height / numTilesH)) * (self.height / numTilesH)
            ...
            for i in range(0,len(self.boids)):
                ...
                if np.size(indexs) > 0:
                    for indx in np.nditer(indexs):
                        difference = [self.boids[i].position[0] - self.boids[indx].position[0], self.boids[i].position[1] - self.boids[indx].position[1]]
                        if (difference[0]**2 + difference[1]**2 > self.boidConfig[0]) or (self.boids[i].angleBetweenBoids(self.boids[i].velocity, difference) > self.boids[i]._viewAngle):
            ...
            for i in range(0, len(self.boids)):
                ...
                if labels[i] != -1:
                    groups[labels[i]].append(i)
                    if self.boids[i].position[0] < clusterBounds[labels[i],0]:
                        clusterBounds[labels[i],0] = self.boids[i].position[0]
                    if self.boids[i].position[0] > clusterBounds[labels[i],1]:
                        clusterBounds[labels[i],1] = self.boids[i].position[0] + 1
                    if self.boids[i].position[1] < clusterBounds[labels[i],2]:
                        clusterBounds[labels[i],2] = self.boids[i].position[1]
                    if self.boids[i].position[1] > clusterBounds[labels[i],3]:
                        clusterBounds[labels[i],3] = self.boids[i].position[1] + 1
            ...
            for i in range(n_clusters_): #Tile for each cluster
                if len(groups[i]) < self.reclusterNum:
                    ...
                    for k in range(0, len(groups[i])):
                        x = int((self.boids[groups[i][k]].position[0] - clusterBounds[i,0]) // (width / numTilesW))
                        x = np.max([np.min([numTilesW-1,x]),0])
                        y = int((self.boids[groups[i][k]].position[1]  - clusterBounds[i,2]) // (height / numTilesH))
    ...
    def CreateBoids(self, width, height, num, boidConfig):
        ...
        for i in range(num):
            ...
            boids.append(Boid([width/2, height/2], [width,height], [x, y], [0, 0, 255], num, boidConfig))
        return boids
